...
std::vector<double> energy(const char* infile, double nX_lower, double nX_upper){

  double ecut = nX_upper;
  double ecut2 = nX_lower;
  ...
  for(int i=1; i<nentries; i++){
      ...
      //     removed++;
      // }
      if((n100>ecut or n100<ecut2) && subid==0){
          //remove event
...
int main(int argc, char** argv){
  ...
  double nX_lower = std::stod(argv[2]);
  double nX_upper = std::stod(argv[3]);
  std::vector<double> energy_cut = energy(infile,nX_lower,nX_upper);
